...
uint8_t broadcastAddress[] = {0x40, 0xF5, 0x20, 0x06, 0xFB, 0x24};
  ...
  if (!modem.gprsConnect(apn, gprsUser, gprsPass)) {
    ...
    if (!client.connect(server, port)) {
      ...
      String httpRequestData = "api_key=" + apiKeyValue + "&Tegangan1=" + float(teganganPanel)
                               ...
                               + "&Suhu1=" + float(myData.temp1) + "&Suhu2=" + float(myData.temp2) + "&Kelembaban1=" + float(myData.humA1)
                               + "&Kelembaban2=" + float(myData.humA2) + "&Kelembaban3=" + float(myData.humB1) 
                                + "&Kelembaban4=" + float(myData.humB2) + "";
